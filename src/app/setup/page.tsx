...
export default function SetupPage() {
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 2:
        return formData.totalCreditHours !== "" && formData.completedCreditHours !== ""
      case 3:
                  ...
                  <div className="bg-muted rounded-lg p-4 mt-4">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-medium text-foreground">
                        {Math.round((parseInt(formData.completedCreditHours) / parseInt(formData.totalCreditHours)) * 100)}%
                      </span>
                    </div>
                    <Progress 
                      value={(parseInt(formData.completedCreditHours) / parseInt(formData.totalCreditHours)) * 100} 
                      className="h-2"
                    />
                    <p className="text-xs text-muted-foreground mt-2">
                      {parseInt(formData.totalCreditHours) - parseInt(formData.completedCreditHours)} credits remaining
                    </p>
                  </div>
                )}
              </div>
