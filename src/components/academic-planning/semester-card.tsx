...
export function SemesterCard({
  ...
  const renderStatus = () => {
    switch (semester.status) {
      case "planned": return <Badge variant="outline" className="bg-muted/50 text-muted-foreground font-normal border-border/50 shadow-none">{tr(t.academicPlanning.statusPlannedLabel)}</Badge>;
      case "current": return <Badge variant="secondary" className="bg-blue-500/10 text-blue-700 font-medium shadow-none">{tr(t.academicPlanning.statusInProgressLabel)}</Badge>;
      case "completed": return <Badge variant="default" className="bg-emerald-500/10 text-emerald-700 border-none hover:bg-emerald-500/20 font-medium shadow-none">{tr(t.academicPlanning.statusCompletedLabel)}</Badge>;
    }
  }
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 text-sm pt-2">
              <div className="flex flex-col">
                <span className="text-muted-foreground text-xs uppercase tracking-wider font-semibold">{tr(t.academicPlanning.creditHours)}</span>
              ...
              <div className="flex flex-col">
                <span className="text-muted-foreground text-xs uppercase tracking-wider font-semibold">{tr(t.academicPlanning.semesterAvg)}</span>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => onEditSemester(semester)} className="cursor-pointer">
                <Edit2 className="w-4 h-4 mr-2 text-muted-foreground" /> {tr(t.academicPlanning.editSemesterDetails)}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onDeleteSemester(semester.id)} className="text-destructive focus:bg-destructive/10 cursor-pointer">
                <Trash2 className="w-4 h-4 mr-2" /> {tr(t.academicPlanning.deleteSemester)}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <CardContent className="p-4 flex-1 flex flex-col gap-3 bg-card">
        {courses.length === 0 ? (
          <div className="text-center py-10 px-4 text-sm text-muted-foreground border border-dashed rounded-xl border-border/60 bg-muted/20">
            <p className="mb-2">{tr(t.academicPlanning.noCoursesYet)}</p>
            <Button variant="link" onClick={() => onAddCourse(semester.id)} className="h-auto p-0">
              {tr(t.academicPlanning.addCourseLink)}
            </Button>
          </div>
        ) : (
