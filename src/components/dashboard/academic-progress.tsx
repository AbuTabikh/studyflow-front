...
export function AcademicProgress({ completionPercentage, passedCredits, requiredCredits }: AcademicProgressProps) {
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Button variant="ghost" size="sm" asChild className="hidden sm:flex -mr-2 text-muted-foreground hover:text-primary transition-colors">
            <Link href="/academic-planning">
              {tr(t.academicProgress.openPlanner)} <ArrowRight className="ml-1 w-4 h-4" />
            </Link>
        </Button>
      </CardHeader>
      ...
      <CardContent className="flex-1 flex flex-col justify-center gap-6 pt-4">
        ...
        <div className="flex justify-between items-end border-b border-border/60 pb-5 relative z-10 pointer-events-none sm:pointer-events-auto">
          <div>
            <p className="text-sm font-medium text-muted-foreground mb-1 flex items-center gap-1">
              <Percent className="w-3.5 h-3.5" /> {tr(t.academicProgress.progress)}
            </p>
          ...
          <div className="text-right">
            <p className="text-sm font-medium text-muted-foreground mb-1">{tr(t.academicProgress.passedHours)}</p>
        ...
        <div className="space-y-2.5 relative z-10 pointer-events-none sm:pointer-events-auto">
          <div className="flex justify-between text-sm font-medium">
            <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md dark:bg-emerald-500/10">{tr(t.academicProgress.keepItUp)}</span>
            <span className="text-muted-foreground">{remainingCredits} {tr(t.academicProgress.hoursLeft)}</span>
          </div>
