...
import { isTaskOverdue } from "@/lib/tasks/utils";
    ...
    <Card className="flex flex-col border-none shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Button variant="ghost" size="sm" asChild className="hidden sm:flex -mr-2 text-muted-foreground hover:text-primary transition-colors">
          <Link href="/tasks">
            {tr(t.highPriorityTasks.viewAll)} <ArrowRight className="ml-1 w-4 h-4" />
          </Link>
        </Button>
      </CardHeader>
      ...
      <CardContent className="flex-1 flex flex-col gap-3 pt-2">
        {tasks.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-8 gap-3">
            ...
            <p className="text-sm text-muted-foreground">{tr(t.highPriorityTasks.noUrgent)}</p>
            <Button size="sm" variant="default" asChild className="rounded-full shadow-none">
              <Link href="/tasks"><Plus className="w-3.5 h-3.5" /> {tr(t.highPriorityTasks.addTask)}</Link>
            </Button>
          </div>
        ) : (
          <>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {task.dueDate && (
                    <p className={cn("text-xs mt-0.5", isTaskOverdue(task) ? "text-red-600" : "text-muted-foreground")}>
                      {isTaskOverdue(task) ? `⚠ ${tr(t.highPriorityTasks.overdue)} · ` : `${tr(t.highPriorityTasks.due)} `}
                      {new Date(task.dueDate).toLocaleDateString(lang === "ar" ? "ar-SA" : "en-US", { month: "short", day: "numeric" })}
                    </p>
                  )}
                </div>
                {onStatusChange && (
                  <button onClick={() => onStatusChange(task.id, "done")} title={tr(t.highPriorityTasks.markDone)} className="opacity-0 group-hover/item:opacity-100 transition-opacity text-muted-foreground hover:text-emerald-600 shrink-0">
            ...
            <Button variant="link" size="sm" asChild className="mt-1 ml-auto text-muted-foreground hover:text-primary self-end">
              <Link href="/tasks">{tr(t.highPriorityTasks.viewAllTasks)} <ArrowRight className="ml-1 w-3.5 h-3.5" /></Link>
            </Button>
          </>
        )}
      </CardContent>
