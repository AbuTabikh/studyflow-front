    ...
    <Card className="flex flex-col h-full border-none shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-semibold flex items-center gap-2">
          ...
          {tr(t.selfLearningProgress.title)}
        </CardTitle>
      </CardHeader>
      <CardContent className="flex-1 flex flex-col justify-between pt-4">
        {currentPlan ? (
          <div className="space-y-4">
            <div className="p-4 rounded-xl border border-border/50 bg-gradient-to-br from-card to-muted/30">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-xs font-medium text-primary mb-1 uppercase tracking-wider">{tr(t.selfLearningProgress.currentPlan)}</p>
              ...
              <div className="space-y-1.5 mt-2">
                <div className="flex justify-between text-xs font-semibold">
                  <span>{tr(t.selfLearningProgress.courseProgress)}</span>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center py-4 gap-2 border-dashed border-2 rounded-xl border-border/40">
            ...
            <p className="text-xs text-muted-foreground">{tr(t.selfLearningProgress.noActivePlan)}</p>
          </div>
        )}
        ...
        <div className="mt-6">
          <Button variant="default" className="w-full gap-2 group" asChild>
            <Link href="/self-learning">
              {currentPlan ? tr(t.selfLearningProgress.continueLearning) : tr(t.selfLearningProgress.explorePlans)}
