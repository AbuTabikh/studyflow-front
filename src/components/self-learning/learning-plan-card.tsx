...
export function LearningPlanCard({ plan, onEdit, onDelete }: PlanCardProps) {
  ...
  const STATUS_BADGE: Record<LearningPlan["status"], { label: string; className: string; icon: React.ReactNode }> = {
    active:    { label: tr(t.selfLearning.active),     className: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-0",     icon: <Zap className="w-3 h-3" /> },
    planned:   { label: tr(t.selfLearning.planned),    className: "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border-0",    icon: <BookOpen className="w-3 h-3" /> },
    completed: { label: tr(t.selfLearning.completed),  className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 border-0", icon: <CheckCircle2 className="w-3 h-3" /> },
    paused:    { label: tr(t.selfLearning.paused),     className: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 border-0", icon: <PauseCircle className="w-3 h-3" /> },
  };
      ...
      <CardContent className="pl-6 pr-4 py-5">
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>{plan.stages.length > 0 ? `${plan.stages.filter(s=>s.status==="completed").length}/${plan.stages.length} ${tr(t.selfLearning.planCardStages)}` : tr(t.selfLearning.planCardNoStages)}</span>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                ...
                {tr(t.selfLearning.planCardStarted)} {formatDate(plan.startDate)}
              </span>
              {plan.endDate && (
                <span className="flex items-center gap-1">
                  <Calendar className="w-3 h-3" />
                  {tr(t.selfLearning.planCardEnds)} {formatDate(plan.endDate)}
                </span>
              )}
              {activeStage && (
                <span className="text-blue-600 dark:text-blue-400 font-medium">
                  {tr(t.selfLearning.planCardActiveStage)} {activeStage}
                </span>
              )}
              {nextMilestone && (
                <span className="text-amber-600 dark:text-amber-400 font-medium">
                  🏁 {tr(t.selfLearning.planCardNextMs)} {nextMilestone.title}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            >
              {tr(t.selfLearning.planCardOpen)} <ArrowRight className="w-3.5 h-3.5" />
            </Button>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => router.push(`/self-learning/${plan.id}`)}>
                  <ArrowRight className="mr-2 h-4 w-4" /> {tr(t.selfLearning.planCardOpenPlan)}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onEdit(plan)}>
                  <Pencil className="mr-2 h-4 w-4" /> {tr(t.selfLearning.planCardEditPlan)}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onDelete(plan.id)} className="text-red-600 focus:text-red-600">
                  <Trash2 className="mr-2 h-4 w-4" /> {tr(t.selfLearning.planCardDeletePlan)}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
